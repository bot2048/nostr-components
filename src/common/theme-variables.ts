...
// Common CSS variables for all components
...
// Type for theme values that have light/dark variants
type ThemeSpecificValue = {
  light: string;
  dark: string;
};
...
export function getThemeValue(
  theme: Theme,
  property: keyof typeof themeVariables
) {
  const value = themeVariables[property];

  if (isThemeSpecific(value)) {
    return value[theme];
  }
...
// Common SVG/assets that are used across components
...
export function getLoadingNostrich(
  ...
  const imageName = `${theme === 'dark' ? 'light' : 'dark'}-nostrich-running.gif`;
