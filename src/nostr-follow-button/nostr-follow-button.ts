...
export default class NostrFollowButton extends HTMLElement {
        ...
        try {
          ...
          if (!userToFollowNpub && !userToFollowNip05 && !userToFollowPubkey) {
            ...
            if (userToFollowPubkey) {
              ...
              const userFromNip05 =
                await ndk.getUserFromNip05(userToFollowNip05);
              if (userFromNip05) {
  ...
  render() {
    ...
    const sanitizedErrorMessage = this.isError
      ? sanitizeText(this.errorMessage)
      : '';
  ...
  ${this.isError ? `<small>${sanitizedErrorMessage}</small>` : ''}
  </div>
  `;
